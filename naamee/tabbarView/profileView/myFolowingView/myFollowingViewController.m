...
@interface myFollowingViewController ()<UITableViewDataSource,UITableViewDelegate>
{
    UITableView *fTable;
...
-(void)getList
{
    ...
    [manager POST:@"http://naamee.com/api/webservices/index.php?format=json" parameters:params progress:nil success:^(NSURLSessionTask *task, id responseObject) {
        NSDictionary* json = [NSJSONSerialization JSONObjectWithData:responseObject
                                                             options:kNilOptions
                                                               error:nil];
        NSLog(@"JSON: %@", [json valueForKey:@"iamfollowing"]);
        [Helper hideIndicatorFromView:self.view];
        
        if ([[json objectForKey:@"success"]integerValue]==1)
        {
            self->fList=[[json valueForKey:@"iamfollowing"]mutableCopy];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([[[fList objectAtIndex:indexPath.row]valueForKey:@"following_status"]intValue]==0)
    {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    [userDefaults setObject:[[fList objectAtIndex:indexPath.row]valueForKey:@"following"] forKey:@"otherUserId"];
...
-(void)follow_Action:(UIButton*)btn
{
    ...
    int  followingOrNot = [[[fList objectAtIndex:btn.tag-1]valueForKey:@"following_status"]intValue];
    if(followingOrNot==0)
    {
        ...
        NSDictionary *params = @{
                                 @"method"       : @"addFollowers",
                                 @"userid"       : [[NSUserDefaults standardUserDefaults]valueForKey:@"userid"],
                                 @"toWhomFollow"       : [[fList objectAtIndex:btn.tag-1]valueForKey:@"following"],
