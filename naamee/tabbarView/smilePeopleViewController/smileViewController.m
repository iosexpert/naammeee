...
@interface smileViewController ()<UITableViewDataSource,UITableViewDelegate>
{
    UITableView *fTable;
...
-(void)getList
{
    ...
    NSDictionary *params = @{
                             ...
                             @"postid"         :[[NSUserDefaults standardUserDefaults] objectForKey:@"likedpostid"],
    ...
    [manager POST:@"http://naamee.com/api/webservices/index.php?format=json" parameters:params progress:nil success:^(NSURLSessionTask *task, id responseObject) {
        ...
        {
            self->fList=[[json valueForKey:@"likePostUsrDetails"]mutableCopy];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    folowBtn.imageEdgeInsets = UIEdgeInsetsMake(0, 41, 0, 0);
    ...
    if([[[fList objectAtIndex:indexPath.row]valueForKey:@"following"]intValue]==0)
    {
        if([[[NSUserDefaults standardUserDefaults]valueForKey:@"userid"]intValue] == [[[fList objectAtIndex:indexPath.row]valueForKey:@"userid"]intValue]){
            folowBtn.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0, 0);
            folowBtn.titleEdgeInsets = UIEdgeInsetsMake(0, 0, 0, 0);
             [folowBtn setTitle:@"My Account" forState:UIControlStateNormal];
        }else{
...
-(void)follow_Action:(UIButton*)btn
{
    ...
    if(followingOrNot==0)
    {
        ...
        NSDictionary *params = @{
                                 @"method"       : @"addFollowers",
                                 @"userid"       : [[NSUserDefaults standardUserDefaults]valueForKey:@"userid"],
                                 @"toWhomFollow"       : [[fList objectAtIndex:btn.tag-1]valueForKey:@"userid"],
